...
char *spider_get_string_between_quote(
  ...
  *end_ptr = '\0';
  ...
  if (connect_string_parse)
    connect_string_parse->set_param_value(start_ptr, start_ptr + strlen(start_ptr) + 1);
...
int spider_create_string_list(
  ...
  *list_length = 0;
  connect_string_parse->init_param_value();
  if (!str)
  {
    *string_list = NULL;
    DBUG_RETURN(0);
  ...
  connect_string_parse->set_param_value(tmp_ptr3,
...
int spider_create_long_list(
  ...
  *list_length = 0;
  ...
  connect_string_parse->set_param_value(tmp_ptr,
...
int st_spider_connect_string_parse::print_param_error()
{
  if (start_title_ptr)
  {
    ...
    int error_num = ER_SPIDER_INVALID_CONNECT_INFO_NUM;
