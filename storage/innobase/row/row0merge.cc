...
@param[in]	default_cols	default values of added, changed columns, or NULL
...
end_of_index:
		...
		row = row_build_w_add_vcol(ROW_COPY_POINTERS, clust_index,
					   ...
					   default_cols, add_v, col_map, &ext,
					   row_heap);
		ut_ad(row);
		...
		for (ulint i = 0; i < n_nonnull; i++) {
			...
			if (dfield_is_null(field)) {
				dfield_t default_field
					= default_cols->fields[nonnull[i]];
				...
				dfield_set_data(field, default_field.data,
						default_field.len);
			}
		}
	...
	bool			drop_historical)
{
	...
	error = row_merge_read_clustered_index(
		...
		n_indexes, default_cols, add_v, col_map, add_autoinc,
